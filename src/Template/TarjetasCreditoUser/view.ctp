<?php
/**
  * @var \App\View\AppView $this
  */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Tarjetas Credito User'), ['action' => 'edit', $tarjetasCreditoUser->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Tarjetas Credito User'), ['action' => 'delete', $tarjetasCreditoUser->id], ['confirm' => __('Are you sure you want to delete # {0}?', $tarjetasCreditoUser->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Tarjetas Credito User'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Tarjetas Credito User'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Users'), ['controller' => 'Users', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New User'), ['controller' => 'Users', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="tarjetasCreditoUser view large-9 medium-8 columns content">
    <h3><?= h($tarjetasCreditoUser->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('User') ?></th>
            <td><?= $tarjetasCreditoUser->has('user') ? $this->Html->link($tarjetasCreditoUser->user->presentacion, ['controller' => 'Users', 'action' => 'view', $tarjetasCreditoUser->user->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($tarjetasCreditoUser->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Numero') ?></th>
            <td><?= $this->Number->format($tarjetasCreditoUser->numero) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('VencimientoMes') ?></th>
            <td><?= $this->Number->format($tarjetasCreditoUser->vencimientoMes) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('VencimientoAnio') ?></th>
            <td><?= $this->Number->format($tarjetasCreditoUser->vencimientoAnio) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('CodSeguridad') ?></th>
            <td><?= $this->Number->format($tarjetasCreditoUser->codSeguridad) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($tarjetasCreditoUser->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($tarjetasCreditoUser->modified) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Active') ?></th>
            <td><?= $tarjetasCreditoUser->active ? __('Yes') : __('No'); ?></td>
        </tr>
    </table>
</div>
